...
# Script name: _fzf-helper
# Description: A helper script for the other scripts in the collection.
# Dependencies:
# GitLab: https://www.gitlab.com/dwt1/fzfscripts
# License: https://www.gitlab.com/dwt1/fzfscripts/LICENSE
...
get_config() {
  ...
  _config_files+=( "${HOME}/scripts/fzfScripts" )
  ...
  _config_files+=( "/etc/fzfscripts/config/config" )
...
check_updated_config() {
  ...
  local _config_file
  _base_file=-1
  [[ -f /etc/fzfscripts/config/config ]] && _base_file="/etc/fzfscripts/config/config"
  _local_conf="$(get_local_config)"
